...
write("model {
  for (i in 1:N) {
    ...
    for (j in 1:2)  {
      mu[i,j]<-alpha[j]+beta[x1[i],j]+beta3[j]*x3[i] +species[sp[i],j]
...
jagsdat12=list(Y=cbind(log(dat1$SCL),log(dat1$K)),x1=as.numeric(dat1$Method),
              x3=abs(dat1$Lat),
...
params12=c("alpha","beta","beta3","species","exp.mean",
          "sp.sd","sp.cor","intercept"
          ,"mu","resid","Sigma","LL")

res12=jags(jagsdat12,init1,params12,model.file="MVNspLat.txt",
          n.chains=2,n.iter=120000,n.burnin=10000,n.thin=4)
write.csv(res12$BUGSoutput$summary,file="Model12.csv")
max(res12$BUGSoutput$summary[,"Rhat"])
min(res12$BUGSoutput$summary[,"n.eff"][res12$BUGSoutput$summary[,"n.eff"]>1])

#Make row in summary table for the run
run=12
restab=data.frame(Model=run)
restab$n.eff[1]=min(res12$BUGSoutput$summary[,"n.eff"][res12$BUGSoutput$summary[,"n.eff"]>1])
restab$Rhat[1]=round(max(res12$BUGSoutput$summary[,"Rhat"]),3)
restab$P.value[1]=NA
restab$DIC[1]=res12$BUGSoutput$DIC 
restab$pDIC[1]=res12$BUGSoutput$pD 
LLrows=paste0("LL[",1:jagsdat12$N,"]")
a=res12$BUGSoutput$sims.matrix[,LLrows]
b=waic(a)
...
restab$deviance[1]=res12$BUGSoutput$summary["deviance","mean"]
restab
...
#Plot residuals
residrows=paste0("resid[",rep(1:jagsdat12$N,2),",",c(rep(1,jagsdat12$N),rep(2,jagsdat12$N)),"]")
murows=paste0("mu[",rep(1:jagsdat12$N,2),",",c(rep(1,jagsdat12$N),rep(2,jagsdat12$N)),"]")
df=data.frame(Expected=res12$BUGSoutput$summary[murows,"mean"],
              Residuals=res12$BUGSoutput$summary[residrows,"mean"],
              Variable=c(rep("Linf",jagsdat12$N),rep("K",jagsdat12$N)),
              k=pareto_k_values(b))
ggplot(df)+geom_point(aes(x=Expected,y=Residuals))+geom_abline(intercept=0,slope=0)+facet_wrap(~Variable,nrow=2, scales = "free")
ggplot(df)+geom_point(aes(x=Expected,y=k))+geom_abline(intercept=0,slope=0)+facet_wrap(~Variable,nrow=2, scales = "free")

# Look at variance parameters
sigrows=c("Sigma[1,1]","Sigma[1,2]","Sigma[2,1]","Sigma[2,2]")
a=res12$BUGSoutput$summary[sigrows,"50%"]
covmatError=matrix(a,2,2)
covmatSpecies=covmatPopulation=matrix(0,2,2)
covmatSpecies[1,1]=res12$BUGSoutput$summary["sp.sd[1]","50%"]^2
covmatSpecies[2,2]=res12$BUGSoutput$summary["sp.sd[2]","50%"]^2
covmatSpecies[1,2]=covmatSpecies[2,1]=res12$BUGSoutput$summary["sp.sd[2]","50%"]*res12$BUGSoutput$summary["sp.sd[1]","50%"]*res12$BUGSoutput$summary["sp.cor","50%"]
covmatError
covmatSpecies
